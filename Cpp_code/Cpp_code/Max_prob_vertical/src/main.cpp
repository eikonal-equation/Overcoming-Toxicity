...
int main()
{
	...
	const double gDefeat_thres = 0.01;
	const double gVictory_thres = 1.0 - gDefeat_thres;
	const double gTol = 1e-6;
	//=============================================Start of Our PDE Solver=====================================================
	auto start = chrono::high_resolution_clock::now();
	cout << "Running Max Porb vertical Solver: " << endl;
	...
	for (int ii = 0; ii < 6; ii++){
		for (int jj = 0; jj < 5; jj++){
			if (ii > 0 || jj > 0){
				...
				myExample.writeDomainToFile("Max_prob_vertical");
	...
	// string filename1 = "ultmap.dat";
	// io::writeToFile2D<double>(filename1, ult_map);
