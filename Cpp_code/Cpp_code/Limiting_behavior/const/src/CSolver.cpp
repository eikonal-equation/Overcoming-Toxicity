			...
			if (aTime_slice == fM - 1) { // terminal condition
				...
				if (j == 0) {//zero population
					...
					auto system_with_ctrl = [this](const state_type& state, state_type& dstate_dt, double t) {
						this->ode_system(state, dstate_dt, t, 1); };
					//integrate the ode
...
void PDE_Solver::precompute_coeff(array_3D& fullstate_loc_mat, array_3D_int& fullstate_ind_mat)
{
	int a = 1; //constitutive killers
	for (int i = 1; i < fNx; i++) { // x loop
		...
		for (int j = 1; j < fNy + 1; j++) {// y loop
			...
			for (int ii = 0; ii < 2; ii++) {
				...
				if (future_state[ii] > 1 - 10*fTol){future_state[ii] = 1;}
				else if (future_state[ii] < fTol){future_state[ii] = 0;}
			}
			...
			for (int jj = 0; jj < 2; jj++) {
				fullstate_loc_mat[i][j][jj] = future_state[jj];
				fullstate_ind_mat[i][j][jj] = future_indx[jj];
			}
...
inline double PDE_Solver::TimeMarch_single_point(const int aI, const int aJ,
	const array_3D& afullstate_loc_mat, const array_3D_int& afullstate_ind_mat, CGrid_2D& aGrid)
{


	//extract loc, indx, and tau
	int x_indx = afullstate_ind_mat[aI][aJ][0];
	int y_indx = afullstate_ind_mat[aI][aJ][1];

	double x_loc = afullstate_loc_mat[aI][aJ][0];
	double y_loc = afullstate_loc_mat[aI][aJ][1];
...
array<double,2> PDE_Solver::Ite_single_point(const int aI, const int aJ, CGrid_2D& aGrid)
{
	...
	if (aI == 0 || aI == fNx){
		...
		//double aVal_right = fMyN_lim_old(aI,N_indx);
		//double aVal_left = fMyN_lim_old(aI,N_indx - 1);
...
array<double,2> PDE_Solver::Ite_single_point_one_time_update(const int aI, const int aJ, CGrid_2D& aGrid)
{
   ...
   if (aI == 450 && aJ == 75){cout << f << " , " << N << " , " << newN << endl;}
	...
	if (aI == 0 || aI == fNx){
		...
		if (N_indx == 1){
			array<double, 2>  aVal_int_f = aGrid.Stencil_for_Linear_Interp(fMyfreq_map, f_indx, N_indx);
			double aVal_f = aGrid.Linear_Interp(aVal_int_f, f_indx, f);
			array<double, 2>  aVal_int_N = aGrid.Stencil_for_Linear_Interp(fMyN_map, f_indx, N_indx);
			double aVal_N = aGrid.Linear_Interp(aVal_int_N, f_indx, f);
			array<double,2> returnVal{aVal_f, aVal_N};
	...
	for (int i = 0; i < fNx + 1; i++) { // x loop
		for (int j = 1; j < fNy + 1; j++) {// y loop
			...
			if (this_change < fTol && !fCheck[i][j]) {
				...
				if (fMyfreq_lim_new[i][j] > 1-10*fTol) {
					fIterMap[i][j] = aIterNum ;
					fCheck[i][j] = true;
				}
				else if (fMyfreq_lim_new[i][j] < 10*fTol) {
					fIterMap[i][j] = -aIterNum;
					fCheck[i][j] = true;
				}
...
void PDE_Solver::Iterative_map(CGrid_2D& aGrid) {
	...
	string filename = "lastslice.dat";
	io::writeToFile2D<double>(filename, fMyfreq_lim_new);
...
void PDE_Solver::Main_Solver() {
	// Initialization
	array_3D myfull_loc_mat(boost::extents[fNx + 1][fNy + 1][2]);
	array_3D_int myfull_ind_mat(boost::extents[fNx + 1][fNy + 1][2]);
	CGrid_2D myGrid(fDx, fDy, fx0, fy0, fNx, fNy);

	string filename1 = "Max_freq_finite_horizon_const_valuefn.dat";
	...
	//io::writeToFile2D<double>(filename1, fMyValfn_current);
}
...
void PDE_Solver::Inf_limit_Solver() {
	...
	string filename1 = "Max_freq_finite_horizon_inf_limit_f_const.dat";
	string filename2 = "Max_freq_finite_horizon_inf_limit_N_const.dat";
...
//#pragma omp parallel for schedule(static,1)
	...
	//Iterative_map(myGrid);
	  ...
	  string filename1 = "limit_iter_rho" + aStr2 + "_T" + aStr + ".dat";
	  io::writeToFile2D<int>(filename1, fIterMap);
	  string filename2 = "limit_f_rho" + aStr2 + "_T" + aStr + ".dat";
	  io::writeToFile2D<double>(filename2, fMyfreq_lim_new);
	  string filename3 = "F_rho" + aStr2 + "_T" + aStr + ".dat";
	  io::writeToFile2D<double>(filename3, fMyValfn_current);
	  string filename4 = "N_rho" + aStr2 + "_T" + aStr + ".dat";
	  io::writeToFile2D<double>(filename4, fMyValfn_current_N);

	 //string filename3 = "limit_check_rho" + aStr2 + "_T" + aStr + ".dat";
	 //io::writeToFile2D<bool>(filename3, fCheck);
	// cout << aStr2 << endl;
}
