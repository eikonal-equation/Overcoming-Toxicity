...
void PDE_Solver::InitializeMat(int aTime_slice) {
	for (int i = 0; i < fNx + 1; i++) { // x loop
		...
		for (int j = 0; j < fNy + 1; j++) {// y loop
			...
			fMyPolicy_current[i][j] = 1; // "a = 1" for constitutive killers!
		}
	}
}
			...
			if (aTime_slice == fM - 1) { // terminal condition
				...
				if (j == 0) {//zero population at the current slice
					...
					auto system_with_ctrl = [this](const state_type& state, state_type& dstate_dt, double t) {
						this->ode_system(state, dstate_dt, t, 1); }; // "a= 1" for constitutive killers
					//integrate the ode
...
// It fills the 3D fullstate_loc_mat with the future state locations for each (i,j) pair, for each state pair (x,y) in the grid.
// and fullstate_ind_mat with the corresponding indices.
// 3D: i -> x ; j -> y ; jj (0,1) -> (x,y); a == 1 (constitutive killers)
void PDE_Solver::precompute_coeff(array_3D& fullstate_loc_mat, array_3D_int& fullstate_ind_mat)
{
	...
	for (int i = 1; i < fNx; i++) { // x loop
		...
		for (int j = 1; j < fNy + 1; j++) {// y loop
			...
			array<double, 2> drift; // drift vector (df, dN) for the current state (fi, Nj) with a == 1
			
			// start with the current state (xi, yj)
			future_state[0] = xi;
			future_state[1] = yj;
			// compute the drift vector based on the current state and a == 1
...
// afullstate_loc_mat: 3D array containing the future state locations for each (i,j) pair for each state pair
// afullstate_ind_mat: 3D array containing the corresponding indices for each (i,j) pair for each state pair
// aGrid: CGrid_2D object that provides the bilinear interpolation functionality.
// Output: a double scalar containing the value function at the current grid point (i, j) for a == 1.
...
inline double PDE_Solver::TimeMarch_single_point_N(const int aI, const int aJ,
	const array_3D& afullstate_loc_mat, const array_3D_int& afullstate_ind_mat, CGrid_2D& aGrid)
{
	// Extract pre-computed loc and indx
	int x_indx = afullstate_ind_mat[aI][aJ][0];
	int y_indx = afullstate_ind_mat[aI][aJ][1];
...
void PDE_Solver::Main_Solver() {
	...
	string filename1 = "Regular_constitutive_valuefn.dat";
	...
	for (int t = fM - 1; t >= 0; t--) // (backwards in time)
	{
		...
		InitializeMat(t);
		// Initial write of the value function to file
		if (t == fM - 1) {
		...
		// Append the computed value function to the output file
...
void PDE_Solver::Inf_limit_Solver() {
	...
	string filename1 = "Regular_constitutive_limit_f.dat";
	string filename2 = "Regular_constitutive_limit_N.dat";
...
void PDE_Solver::Inf_limit_Solver_OutputIter() {
	...
	string filename1 = "Regular_constitutive_limit_iter_rho" + aStr2 + "_T" + aStr + ".dat";
	io::writeToFile2D<int>(filename1, fIterMap);
	string filename2 = "Regular_constitutive_limit_f_rho" + aStr2 + "_T" + aStr + ".dat";
