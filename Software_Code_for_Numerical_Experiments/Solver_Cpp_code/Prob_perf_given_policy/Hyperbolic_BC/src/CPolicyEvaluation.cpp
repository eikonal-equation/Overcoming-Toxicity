 ...
 *============================================================================*/
  ...
  * under the condition of a strategically-optimal control problem.
  * It specifically handles the case of a hyperbolic boundary condition.
  * We use the Eigen library for both direct sparse LU solver and iterative approximate solver.
  *
  *============================================================================*/
	...
	const array_4D& afullstate_loc_mat, const array_4D_int& afullstate_ind_mat, array_2D& aValfn) 
{
	...
	for (int i = 0; i < fNx + 1; i++) {// x loop
		...
		for (int j = 0; j < fNy + 1; j++) {// y loop
			...
			// For hyperbolic boundary condition, add entries for the adjacent grid points
			if (j > 0 && xi * yj >= fDefeat_thres && xi * yj <= fVictory_thres && aValfn[i][j] != 1 && aValfn[i][j] != 0)
			{
				...
				aYrho_index = afullstate_ind_mat[i][j][current_policy][2];
				aXloc = afullstate_loc_mat[i][j][current_policy][0];
				aYloc = afullstate_loc_mat[i][j][current_policy][1];
				aYrho_loc = afullstate_loc_mat[i][j][current_policy][2];

				x_pos_tr = aXindex * fDx + fx0;
				y_pos_tr = aYindex * fDy + fy0;
				yrho_pos_tr = aYrho_index * fDy + fy0;

				// Add entries (without rho) to the sparse matrix A for the semi-Lagrangian discretization
				...
				// Add entries (with rho) to the sparse matrix A for the semi-Lagrangian discretization
				entries.push_back(Triplet<double>(i * (fNx + 1) + j, (aXindex - 1) * (fNx + 1) + aYrho_index - 1, -fProb_arrival * (yrho_pos_tr - aYrho_loc) / fDy * (x_pos_tr - aXloc) / fDx)); //bottom left
				entries.push_back(Triplet<double>(i * (fNx + 1) + j, (aXindex) * (fNx + 1) + aYrho_index - 1, -fProb_arrival * (yrho_pos_tr - aYrho_loc) / fDy * (aXloc - (x_pos_tr - fDx)) / fDx)); //bottom right
				entries.push_back(Triplet<double>(i * (fNx + 1) + j, (aXindex - 1) * (fNx + 1) + aYrho_index, -fProb_arrival * (aYrho_loc - (yrho_pos_tr - fDy)) / fDy * (x_pos_tr - aXloc) / fDx)); //top left
				entries.push_back(Triplet<double>(i * (fNx + 1) + j, (aXindex) * (fNx + 1) + aYrho_index, -fProb_arrival * (aYrho_loc - (yrho_pos_tr - fDy)) / fDy * (aXloc - (x_pos_tr - fDx)) / fDx)); //top right
			}
			else {
