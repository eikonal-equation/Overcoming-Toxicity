...
public:
	...
	PDE_Solver(int aRefinement_factor, double aMaxPopSize, double aMaxFrequency,
		double aTol, double aEpsilon, double aRks, double aGamma, double aLamb)
	{
		...
		fProb_not_arrival = exp(-fLamb * fTau); // Probability of dilution not arriving (from exponential distribution) at the next time step
	...
	array<double,2> ValueIte_single_point(const int aI, const int aJ,
