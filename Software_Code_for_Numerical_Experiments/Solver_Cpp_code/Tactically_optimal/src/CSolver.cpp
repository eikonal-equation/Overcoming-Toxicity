...
/*==============================================================================
  ...
  * which constructs the PDE solver for the tactically-optimal control problem.
  * It carrys out a Gauss–Seidel Value-Policy-Iteration (GS-VPI) algorithm to 
  * solve for the value function and recover the corresponding tactically-optimal policy 
...
void PDE_Solver::precompute_coeff(array_4D& fullstate_loc_mat, array_4D_int& fullstate_ind_mat)
{
	for (int i = 1; i < fNx; i++) { // x loop
		...
		for (int j = 1; j < fNy + 1; j++) {// y loop
			...
			array<double, 2> drift;
			// Loop over the two controls (0 and 1)
			for (int a = 0; a < 2; a++) {
...
array<double,2> PDE_Solver::ValueIte_single_point(const int aI, const int aJ,
	const array_4D& afullstate_loc_mat, const array_4D_int& afullstate_ind_mat, CGrid_2D& aGrid)
{
	...
	for (int a = 0; a < 2; a++) { // Loop over the two controls (0 and 1)
		...
		// Store the computed value in the return array
		returnVal[a] = aVal;
	}
	return returnVal;
}
...
double PDE_Solver::ValueIte_single_step(const array_4D& afullstate_loc_mat, const array_4D_int& afullstate_ind_mat, CGrid_2D& aGrid)
{
	...
	for (int i = 1; i < fNx; i++) { // x loop
		...
		for (int j = 1; j < fNy + 1; j++) {// y loop
			...
			for (int ii = 0; ii < 2; ii++) {
				w_possible[ii] = fProb_not_arrival * InterpVals[ii] + fProb_arrival * xi;
			}
...
void PDE_Solver::Main_VPI_Solver() {
	...
	CPolicyEvaluation myPEclass(fNx, fNy, fx0, fy0, fDx, fDy,fProb_not_arrival, fProb_arrival);
	...
	string filename1 = "Sto_tactic_valfn_lamb" + str1 + ".dat";
	string filename2 = "Sto_tactic_policy_lamb" + str1 + ".dat";
...
void PDE_Solver::writeDomainToFile(const string aFilename) {
	...
	array_1D params(boost::extents[8]); // Creating boostarray "row vector"
