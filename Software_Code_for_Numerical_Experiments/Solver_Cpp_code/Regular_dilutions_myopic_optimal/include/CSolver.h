 ...
 *============================================================================*/
  ...
  * which implements the PDE solver for the myopic-optimal control problem. 
  * It carrys out time marching (backwards in time) to solve for the value function and recover the 
...
public:
		...
		double aTol, double aEpsilon, double aRks, double aGamma, double aTf, double aRho)
	{
		...
		fIterMap.resize(boost::extents[fNx + 1][fNy + 1]); //Initialize the storage of map of competitive exclusion for the limiting behavior of the myopic-optimal policy
	...
	// It computes the new values of the killer frequency and total population size at each grid point (i, j)
	// using the results from the initial map.
	// In addition, it updates the number of iterations for a single step (iteration).
