...
int main()
{
	...
	// Building the main class of our PDE solver
	...
	cout << "Running the myopic-optimal solver (regular dilutions): " << endl;
	myExample.Main_Solver();

	// Uncomment the following two lines to run the PDE solver + the Infinite limit solver
	//cout << "Running the myopic-optimal solver followed by the infinite limit solver (regular dilutions): " << endl;
	//myExample.Inf_limit_Solver();

	// Uncomment the following two lines to run the PDE solver followed by the infinite limit solver
	// (that also outputs the map of exclusion) for the limiting behavior
	// cout << "Running the myopic-optimal solver followed by the infinite limit solver that also outputs the map of competitive exclusion (regular dilutions): " << endl;
	// myExample.Inf_limit_Solver_OutputIter();


	// Writing Grid Parameters to file
	myExample.writeDomainToFile("Regular_dilutions_myopic_optimal");
